...
// mbid (optional): löst Namensvetter auf — statt über den Namen (der bei Last.fm die
// populärere gleichnamige Identität zieht) gezielt DIESE MusicBrainz-Identität abfragen.
export async function getSimilar(name, { limit = 30, key, mbid } = {}) {
  key ??= await getKey();
  const ck = (mbid ? "mbid:" + mbid : name) + "|" + limit; // eigener Cache-Schlüssel je Identität
  return cached("similar", ck, 14 * 864e5, async () => {
...
// Wie searchArtists, aber mit Zusatzinfos je Treffer (Hörerzahl + Last.fm-URL) — damit die
// Autocomplete bei mehrdeutigen Namen zeigen kann, WELCHER Act gemeint ist (N1). Eigener
// Cache-Schlüssel, damit die reine Namensliste (searchArtists) unberührt bleibt. 1 Tag gecacht.
export async function searchArtistsDetailed(q, { limit = 6, key } = {}) {
  key ??= await getKey();
  return cached("search-det", q + "|" + limit, 1 * 864e5, async () => {
    const url = new URL("https://ws.audioscrobbler.com/2.0/");
    url.searchParams.set("method", "artist.search");
    url.searchParams.set("artist", q);
    url.searchParams.set("api_key", key);
    ...
    const seen = new Set();
    return list
      .filter((a) => a && a.name && !seen.has(a.name.toLowerCase()) && seen.add(a.name.toLowerCase()))
      .slice(0, limit)
      .map((a) => ({ name: a.name, listeners: parseInt(a.listeners, 10) || null, url: a.url || null }));
  });
}
...
// Top-Acts eines Genres/Tags (tag.gettopartists) — für die genre-gefilterte „Überrasch mich".
// Nach Popularität sortiert (populärste zuerst); der Aufrufer kann daraus einen eher unbekannten
// (weiter hinten) ziehen. Alle sind über Last.fm ladbar (Graph ist namensbasiert). 30 Tage gecacht.
export async function getTagArtists(tag, { limit = 50, key } = {}) {
  const t = String(tag || "").trim();
  if (!t) return [];
  key ??= await getKey();
  return cached("tag-artists", t.toLowerCase() + "|" + limit, 30 * 864e5, async () => {
    const url = new URL("https://ws.audioscrobbler.com/2.0/");
    ...
    return arr.map((a) => ({ name: a.name, mbid: a.mbid || null, url: a.url || null })).filter((a) => a.name);
  });
}
