...
export async function createFeedbackIssue({ message, pack, version }) {
  ...
  // U-2c.7: Nutzertext in einen Codeblock einbetten -> kein Markdown-Rendering, keine aktiven
  // @mentions/Links (im Code-Fence lösen @handles keine GitHub-Benachrichtigung aus). Backtick-
  // Sequenzen entschärfen, damit der Text nicht aus dem Fence ausbrechen kann.
  const safe = msg.replace(/```+/g, (m) => "'".repeat(m.length));
  const body = [
    "```text",
    safe,
    "```",
