...
export async function initAuth(dataDir) {
  ...
  if (!SECRET) {
    ...
    // U-2c.9: Session-Secret nur für den Besitzer lesbar (0600) — es signiert alle Login-Cookies.
    if (!SECRET) { SECRET = randomBytes(32).toString("hex"); try { await mkdir(dirname(f), { recursive: true }); await writeFile(f, SECRET, { mode: 0o600 }); } catch {} }
  }
}
...
// U-2b (DSGVO Art. 17): Konto vollständig entfernen — aus dem In-Memory-Store UND persistiert.
// Danach scheitert jede bestehende Session dieses Nutzers (sessionSig kennt den salt nicht mehr),
// und ein zwischenzeitliches persist() kann kein „Geister-Konto" zurückschreiben. true, wenn es das
// Konto gab. Der zugehörige Nutzer-Datenraum wird vom Aufrufer (server.mjs) gelöscht.
