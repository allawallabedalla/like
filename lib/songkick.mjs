...
export async function songkickCoappear(name) {
  ...
  const s = await fetch(`https://api.songkick.com/api/3.0/search/artists.json?apikey=${k}&query=${encodeURIComponent(name)}`);
  if (!s.ok) return null;
  const sj = await s.json();
  const artist = sj.resultsPage?.results?.artist?.[0];
  if (!artist) return null;
  const g = await fetch(`https://api.songkick.com/api/3.0/artists/${artist.id}/gigography.json?apikey=${k}&per_page=50`);
  if (!g.ok) return null;
