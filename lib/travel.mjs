...
// (U-2d) Ort-Namen für den Namensvetter-Vergleich normalisieren: klein, ohne Diakritika
// (ü->u, é->e), ohne Klammer-/Komma-Zusatz — so matcht „Springfield" auch die Wikivoyage-
// Disambiguierung „Springfield (Illinois)" und die Eingabe „Berlin, Deutschland" den Titel „Berlin".
function normPlace(s) {
  return String(s || "")
    .normalize("NFD").replace(/[̀-ͯ]/g, "")
    .toLowerCase()
    .replace(/\s*[(,].*$/, "")   // alles ab erster Klammer/Komma abschneiden (Disambiguierungs-Zusatz)
    .replace(/\s+/g, " ")
    .trim();
}

// (U-2d) Namensvetter-Disambiguierung: aus mehreren Suchtreffern bewusst wählen, statt blind
// den erstbesten zu nehmen. Ein exakter (normalisierter) Namens-Treffer schlägt eine bloße
// Teil-Übereinstimmung; bei mehreren Gleichnamigen gewinnt der bekanntere/vollständigere Artikel
// (Wikivoyage-Byte-Größe + Wortzahl als Bekanntheits-Näherung), erst danach der Suchrang.
// Verhindert, dass ein obskurer gleichnamiger Ort statt des gemeinten landet.
function pickBestPlace(query, hits) {
  const want = normPlace(query);
  let best = null, bestScore = -Infinity;
  (hits || []).forEach((h, i) => {
    if (!h?.title) return;
    const base = normPlace(h.title);
    const exact = base === want;
    const partial = !exact && !!want && (base.startsWith(want) || want.startsWith(base));
    // Vollständigkeit gedeckelt (< Teil-Bonus), damit Exaktheit immer vor bloßer Größe rangiert.
    const size = Math.min((h.size || 0) + (h.wordcount || 0) * 5, 5e5);
    const score = (exact ? 1e9 : partial ? 1e6 : 0) + size - i; // Exaktheit → Vollständigkeit → Rang
...
export async function resolveTitle(name) {
  return cached("voy-title2", name, 14 * 864e5, async () => {
    ...
    for (const lang of ["de", "en"]) {
      try {
        // (U-2d) Mehrere Kandidaten holen (statt srlimit=1) und per pickBestPlace disambiguieren.
        // srprop=size|wordcount liefert die Vollständigkeits-Signale ohne Snippet-Ballast.
        const d = await voyQuery(lang, { action: "query", list: "search", srsearch: name, srlimit: "8", srnamespace: "0", srprop: "size|wordcount" });
        const hits = d.query?.search || [];
        // Reisethemen (keine Orte) aus der Auswahl nehmen; nur wenn sonst nichts bliebe, doch zulassen.
        const places = hits.filter((h) => h?.title && !NON_PLACE.test(h.title));
        const best = pickBestPlace(name, places.length ? places : hits);
