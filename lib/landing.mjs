...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .tagline p { margin: 3px 0 0; font-size: 12.5px; opacity: .78; text-shadow: 0 1px 8px #000b; }
  ...
  /* (U-2e) Primärer CTA + „Als App laden"-Hinweis direkt unter der Tagline — der klare Einstieg.
     Die Tagline selbst ist pointer-events:none; die Links hier bekommen die Klicks zurück. */
  .ctarow { margin-top: 12px; display: flex; gap: 12px; align-items: center; justify-content: center;
    flex-wrap: wrap; pointer-events: auto; }
  .cta { text-decoration: none; border-radius: 11px; white-space: nowrap;
    font: 600 13.5px system-ui, -apple-system, sans-serif; letter-spacing: .01em;
    transition: transform .16s, box-shadow .16s, opacity .16s; }
  .cta:focus-visible { outline: 2px solid #ff8a3d; outline-offset: 2px; }
  .cta-primary { padding: 9px 18px; color: #3a1a05;
    background: radial-gradient(circle at 38% 34%, #ffd9a8, #ff8a3d 78%);
    box-shadow: 0 8px 22px -8px rgba(255,138,61,.6), inset 0 0 0 1px #ffffff2e; }
  .cta-primary:hover, .cta-primary:focus-visible { transform: translateY(-1px);
    box-shadow: 0 10px 28px -8px rgba(255,138,61,.78), inset 0 0 0 1px #ffffff40; }
  .cta-app { padding: 8px 14px; color: var(--fg); opacity: .82; border: 1px solid var(--line);
    background: color-mix(in srgb, var(--card) 80%, transparent); backdrop-filter: blur(8px); }
  .cta-app:hover, .cta-app:focus-visible { opacity: 1; }
  body.flatmode .cta-app { background: #fff; border-color: #e2e2e2; color: #000; opacity: .9; }
  @media (max-height: 560px) { .ctarow { margin-top: 8px; } }
  /* (U-2e) Labs-Freischaltung: schlichtes Inline-Panel statt nativem prompt()/alert(). */
  .labsov { position: fixed; inset: 0; z-index: 20; display: flex; align-items: center; justify-content: center;
    padding: 16px; background: #05070db3; backdrop-filter: blur(3px); opacity: 0; transition: opacity .16s; }
  .labsov.show { opacity: 1; }
  .labsbox { width: min(340px, 100%); padding: 20px; border-radius: 16px;
    background: color-mix(in srgb, var(--card) 96%, transparent); border: 1px solid var(--line);
    box-shadow: 0 20px 60px -18px #000c; transform: translateY(6px); transition: transform .16s; }
  .labsov.show .labsbox { transform: none; }
  .labsbox h2 { margin: 0 0 6px; font-size: 15.5px; font-weight: 650; }
  .labsbox p { margin: 0 0 14px; font-size: 12.5px; opacity: .74; line-height: 1.45; }
  .labsbox input { width: 100%; padding: 10px 12px; border-radius: 10px; border: 1px solid var(--line);
    background: var(--bg); color: var(--fg); font: 14px system-ui, -apple-system, sans-serif; }
  .labsbox input:focus-visible { outline: 2px solid #ff8a3d; outline-offset: 1px; }
  .labserr { min-height: 16px; margin: 8px 2px 0; font-size: 12px; color: #ff6a5a; }
  .labsact { display: flex; gap: 8px; justify-content: flex-end; margin-top: 12px; }
  .labsbtn { border: 0; border-radius: 10px; padding: 9px 16px; cursor: pointer;
    font: 600 13px system-ui, -apple-system, sans-serif; }
  .labsbtn.ghost { background: transparent; border: 1px solid var(--line); color: var(--fg); }
  .labsbtn.go { color: #3a1a05; background: radial-gradient(circle at 38% 34%, #ffd9a8, #ff8a3d 78%); }
  body.flatmode .labsbox { background: #fff; border-color: #e2e2e2; }
  body.flatmode .labsbox input { background: #fff; color: #000; }
  body.flatmode .labsbtn.ghost { color: #000; border-color: #e2e2e2; }
  footer { position: fixed; left: 0; right: 0; bottom: 14px; text-align: center; font-size: 12.5px; opacity: .72; z-index: 2; }
  ...
  body.flatmode footer { opacity: .72; color: #000; }
  ...
  ${opts.tagline ? `<div class="tagline"><h1>${opts.tagline.h}</h1>${opts.tagline.p ? `<p>${opts.tagline.p}</p>` : ""}<div class="ctarow"><a class="cta cta-primary" href="/?pack=music">Los geht's →</a><a class="cta cta-app" href="https://github.com/allawallabedalla/like/releases/latest" target="_blank" rel="noreferrer">Als App laden</a></div></div>` : ""}
  <canvas id="bg"></canvas>
...
(function () {
  ...
  var D = {
    ...
    // (U-2e) Primärer CTA + Download-Hinweis unter der Tagline
    "Los geht's →": "Get started →", "Als App laden": "Get the app",
    // (U-2e) Inline-Panel für die Labs-Freischaltung
    "Labs freischalten": "Unlock Labs",
    "Labs sind Experimente, die noch nicht öffentlich sind. Mit dem Passwort schaltest du sie frei.": "Labs are experiments that aren't public yet. Enter the password to unlock them.",
    "Passwort": "Password", "Abbrechen": "Cancel", "Freischalten": "Unlock",
  };
  ...
  function tr(s) {
    ...
    // Teilsätze in längeren Knoten (z. B. Footer "🚧 Diese Seite entsteht gerade · v1.2 · alle Domänen …")
    // Beide Teilsätze können im selben Textknoten stehen -> nacheinander ersetzen, nicht früh returnen.
    if (s.indexOf("alle Domänen in einer App") >= 0 || s.indexOf("Diese Seite entsteht gerade") >= 0) {
      var out = s;
      if (out.indexOf("Diese Seite entsteht gerade") >= 0) out = out.replace("Diese Seite entsteht gerade", "This site is still a work in progress"); // FB19/#86
      if (out.indexOf("alle Domänen in einer App") >= 0) out = out.replace("alle Domänen in einer App", D["alle Domänen in einer App"]);
      return out;
    }
    return s;
  }
...
${gated ? `<script>
// (U-2e) Gesperrtes Lab freischalten: schlichtes Inline-Panel statt nativem prompt()/alert().
// Erklärt kurz, was „Labs" sind, nimmt das Passwort entgegen und zeigt Fehler INLINE.
// Zugänglich: role=dialog, Fokus ins Feld, Esc + Außenklick schließen, Fokus kehrt zurück.
(function(){
  var TR = window.LIKE_TR || function(s){ return s; }; // Sprach-Wörterbuch (EN), sonst Durchreiche
  var ov=null, input=null, errEl=null, curHref=null, lastFocus=null;
  function build(){
    ov=document.createElement("div"); ov.className="labsov";
    ov.setAttribute("role","dialog"); ov.setAttribute("aria-modal","true");
    ov.setAttribute("aria-labelledby","labsTitle");
    ov.innerHTML='<div class="labsbox">'
      +'<h2 id="labsTitle">'+TR("Labs freischalten")+'</h2>'
      +'<p>'+TR("Labs sind Experimente, die noch nicht öffentlich sind. Mit dem Passwort schaltest du sie frei.")+'</p>'
      +'<input type="password" autocomplete="off" aria-label="'+TR("Passwort")+'" placeholder="'+TR("Passwort")+'">'
      +'<div class="labserr" role="alert" aria-live="assertive"></div>'
      +'<div class="labsact">'
      +'<button type="button" class="labsbtn ghost" data-x>'+TR("Abbrechen")+'</button>'
      +'<button type="button" class="labsbtn go" data-ok>'+TR("Freischalten")+'</button>'
      +'</div></div>';
    document.body.appendChild(ov);
    input=ov.querySelector("input"); errEl=ov.querySelector(".labserr");
  ...
  function open(href){
    if(!ov) build();
    curHref=href; lastFocus=document.activeElement;
    errEl.textContent=""; input.value=""; ov.classList.add("show");
    requestAnimationFrame(function(){ input.focus(); });
  }
  function close(){
    if(!ov) return; ov.classList.remove("show");
    if(lastFocus && lastFocus.focus) lastFocus.focus();
  }
  function submit(){
    errEl.textContent="";
    fetch("/api/unlock",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({password:input.value})})
      .then(function(r){ if(r.ok){ location.href=curHref; } else { errEl.textContent=TR("Falsches Passwort."); input.focus(); input.select(); } })
      .catch(function(){ errEl.textContent=TR("Freischalten fehlgeschlagen."); });
  }
  document.querySelectorAll(".planet.locked").forEach(function(el){
    var href = el.getAttribute("data-href");
    el.addEventListener("click", function(){ open(href); });
    el.addEventListener("keydown", function(e){ if(e.key==="Enter"||e.key===" "){ e.preventDefault(); open(href); } });
  });
})();
</script>` : ""}
</body></html>`;
