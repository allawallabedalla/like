...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  /* Flat-Modus: Planeten flach wie die Knoten im App-Netz (kein 3D-Verlauf/Glow), Sonne ruhig. */
  .solar.flat .orb { background: hsl(var(--h,210) 42% 58%);
    box-shadow: inset 0 0 0 1px #ffffff22, 0 2px 8px -4px #000; }
  .solar.flat .planet:hover .orb, .solar.flat .planet:focus-visible .orb { transform: scale(1.12);
    box-shadow: inset 0 0 0 1px #ffffff33, 0 3px 12px -4px #000; }
  /* FLAT = hell, exakt wie das App-Netz im Light-Modus: heller Hintergrund, Sternenfeld aus,
     dunkle Labels, „like" als weißer Such-Knoten (schwarzer Rand), Domänen als flache Kugeln. */
  body.flatmode { background: radial-gradient(130% 90% at 72% -12%, #dce7ff 0%, #eef2fb 46%, #f4f6fb 100%) !important; }
  body.flatmode #bg { display: none !important; }
  body.flatmode .plabel { color: #1b2233; text-shadow: 0 1px 6px #ffffffcc; }
  body.flatmode .landtoggle, body.flatmode .themetgl { background: #ffffffcc; border-color: #d4dbe9; color: #1b2233; }
  body.flatmode .landtoggle button { color: #1b2233; }
  body.flatmode .landtoggle button.on { background: #1b2233; color: #fff; }
  body.flatmode .hint { background: #ffffffe6; border-color: #d4dbe9; color: #1b2233; }
  body.flatmode .sun { background: #ffffff; color: #1b2233;
    box-shadow: 0 2px 16px -5px rgba(20,30,60,.3), inset 0 0 0 1.5px #cdd6e6; }
  body.flatmode .sun b { color: #ff8a3d; }
  body.flatmode .solar.flat .orb { background: hsl(var(--h,210) 46% 60%);
    box-shadow: inset 0 0 0 1px #00000018, 0 2px 8px -4px rgba(20,30,60,.35); }
  body.flatmode footer { opacity: .5; color: #1b2233; }
...
(function () {
  ...
  function frame(ts) {
    ...
    if (flat) { nctx.lineWidth = 1.5; nctx.strokeStyle = "rgba(74,120,246,0.6)"; } // Kantenfarbe wie im App-Netz (blau) auf hell
