...
let raFailStreak = 0, raLastAlarm = 0, raLastFail = 0;
// R15 — Abkühlphase bei RA-Störung: Fehler landen bewusst NIE im Datei-Cache, d. h. ohne
// diese Bremse zahlt während eines RA-Ausfalls JEDER Ausbau erneut die volle Retry-Kaskade
// (~5-10 s Hintergrundlast). Ab 2 Fehlschlägen in Folge überspringen neue RA-Anfragen die
// Quelle für 10 Minuten; der erste Erfolg (oder Ablauf) hebt die Sperre auf.
const RA_COOLDOWN_MS = 10 * 60e3;
function raCoolingDown() { return raFailStreak >= 2 && Date.now() - raLastFail < RA_COOLDOWN_MS; }
function raHealth(ok, err) {
  if (ok) { raFailStreak = 0; return; }
  raFailStreak++; raLastFail = Date.now();
...
export async function coappearByName(name) {
  return cached("ra", name, 3 * 864e5, async () => {
    // R15: laufende Abkühlphase -> sofort werfen (wird nicht gecacht; coAppearances fängt
    // das und degradiert wie bei jedem RA-Fehler). Kein einziger Netz-Request fällt an.
    if (raCoolingDown()) throw new Error("RA-Störung — Abkühlphase aktiv, nächster Versuch in wenigen Minuten");
