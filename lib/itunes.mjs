...
// FB2/#81: `norm` strippt Diakritika -> „Magnüm" (frz. HipHop) faltet auf „magnum" und würde
// als „Magnum" (Rockband) durchgehen. Darum ZUERST diakritik-sensitiv exakt matchen (nur Groß-/
// Kleinschreibung + Leerraum egal) und erst als Fallback die gefaltete Variante nutzen.
const strict = (s) => String(s).toLowerCase().replace(/\s+/g, " ").trim();
const pickHit = (results, name) => {
  const rs = (results || []).filter((r) => r.previewUrl);
  const hit = rs.find((r) => strict(r.artistName) === strict(name)) || rs.find((r) => matches(r.artistName, name));
