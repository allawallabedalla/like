...
export async function discoverTag(tag, { limit = 10 } = {}) {
  ...
  return cached("bc-discover", t, 1 * 864e5, async () => {
    return throttled(async () => {
      const res = await fetch(`https://bandcamp.com/api/discover/3/get_web?g=${encodeURIComponent(t)}&s=new&p=0&gn=0&f=all&w=0`, { headers: HEADERS, signal: AbortSignal.timeout(8000) });
      if (!res.ok) throw new Error("Bandcamp HTTP " + res.status);
      const j = await res.json();
      const items = j.items || [];
