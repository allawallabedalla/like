...
export function emptyGraph() {
  return {
    ...
    events: [],
    sources: [],
    lists: [], // Like-Listen (v2.6): benannte Sammel-Buckets; Mitgliedschaft steht je Artist in a.lists[]
  };
}

// Die Default-Liste ist IMMER vorhanden — sie ist das Ziel für „like", solange keine andere
// aktiv ist, und ersetzt den früheren Einzel-Korb (a.basket). `auto:true` = der Client darf den
// Namen lokalisieren (Lineup/Favoriten); ein Umbenennen entfernt das Flag.
export const DEFAULT_LIST_ID = "default";
export function ensureLists(g) {
  g.lists ??= [];
  if (!g.lists.some((l) => l.id === DEFAULT_LIST_ID)) {
    g.lists.unshift({ id: DEFAULT_LIST_ID, name: "Meine Likes", color: "#ff6a00", auto: true, created: Date.now() });
  }
  const valid = new Set(g.lists.map((l) => l.id));
  for (const a of Object.values(g.artists)) {
    // Alt-Korb (a.basket) verlustfrei in die Default-Liste überführen
    if (a.basket) {
      a.lists = Array.isArray(a.lists) ? a.lists : [];
      if (!a.lists.includes(DEFAULT_LIST_ID)) a.lists.push(DEFAULT_LIST_ID);
      delete a.basket;
    }
    if (Array.isArray(a.lists)) {
      a.lists = [...new Set(a.lists.filter((id) => valid.has(id)))]; // verwaiste/ungültige IDs raus
      if (!a.lists.length) delete a.lists;
    } else if (a.lists != null) {
      delete a.lists;
    }
  }
}
...
export async function loadGraph(path) {
  ...
  try {
    ...
    try {
      ...
      g.artists ??= {}; g.edges ??= []; g.events ??= []; g.sources ??= []; g.lists ??= []; g.meta ??= { version: 1 };
      migrate(g);
      return g;
...
function migrate(g) {
  ...
  ensureLists(g); // Like-Listen sicherstellen + Alt-Korb (a.basket) migrieren
}
