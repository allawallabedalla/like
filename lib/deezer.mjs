...
// FB2/#81: `norm` strippt Diakritika -> „Magnüm" faltet auf „magnum" und würde „Magnum" matchen.
// Darum zuerst diakritik-sensitiv exakt (nur Groß-/Kleinschreibung + Leerraum egal), dann Fallback.
...
export async function artistByName(name) {
  return cached("dz-artist", name, 7 * 864e5, async () => {
    ...
    const hit = (d.data || []).find((a) => strict(a.name) === strict(name)) || (d.data || []).find((a) => norm(a.name) === norm(name));
    if (!hit) return null;
...
export async function trackPreviewSearch(name) {
  return cached("dz-preview-trk", name, 14 * 864e5, async () => {
    ...
    const withPrev = (d.data || []).filter((t) => t.preview && t.artist);
    const hit = withPrev.find((t) => strict(t.artist.name) === strict(name)) || withPrev.find((t) => nameMatches(t.artist.name, name));
    if (!hit) return null;
