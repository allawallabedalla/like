...
const DEFAULT_FILE = join(process.env.LIKE_DATA_DIR || dirname(dirname(fileURLToPath(import.meta.url))), "stats.json");
...
// Dateipfad wird pro Aufruf übergeben (nicht global) — sonst könnten sich bei
// Pack-pro-Request mehrere Requests über einen globalen State in die Quere kommen.
export async function loadStats(file = DEFAULT_FILE) {
  try { return JSON.parse(await readFile(file, "utf8")); }
  catch { return {}; }
}

export async function saveStats(fileOrStats, maybeStats) {
  // Rückwärtskompatibel: saveStats(stats) (Default-Datei) ODER saveStats(file, stats).
  const [file, stats] = maybeStats === undefined ? [DEFAULT_FILE, fileOrStats] : [fileOrStats, maybeStats];
  const tmp = file + ".tmp";
