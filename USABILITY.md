...
| Entdecken | `#discoverBtn` / `#mDiscover` | Öffnet das Entdecken-Popover (§6) — dort sitzt auch das **Radar** (FB25). Im Stöbern-Modus = direkt **Streifzug**. | immer |
...
| Feedback | `#feedbackBtn` | ✉ Feedback / Fehler melden (§13). | nur wenn Pushover/Issues eingerichtet |
...
|---|---|---|---|
| **Radar** | `#discRadar` | Öffnet das Geheimtipp-Radar (§7). *(FB25/#93: früher eigener Topbar-Knopf, jetzt ins Entdecken-Popover gebündelt.)* | `features.radar` |
...
- **PNG** (`#exportPng`), **CSV** der markierten Einträge (`#export`, nur Profi; Formel-Injection-sicher).
...
## 11. Löschen-Dialog (`#deleteModal`, N6/FB8)

Öffnet über **Löschen…** (`#resetAll`, Profi). Drei Umfänge (`data-scope`): **Nur unverknüpfte Einträge
aufräumen** (`discovered`, Gemerkte/Notierte bleiben) · **Auftritts- & Wiki-Zusatzdaten zurücksetzen**
(`lineups`) · **Ganze Karte leeren** (`all`) — je mit umfangsspezifischer Sicherheitsabfrage.
...
**Definition of Done:** Jede UI-Änderung (neue/umbenannte/verschobene Elemente, geänderte IDs) pflegt
USABILITY.md mit — die Datei ist die Single Source für Funktions-/UI-Referenz, Intro-Tour und Hilfe.
Ein PR mit UI-Änderung ohne passende USABILITY-Anpassung gilt als unvollständig.
